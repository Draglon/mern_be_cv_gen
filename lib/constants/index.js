export const MAX_TEXTAREA_CONTENT_LENGTH = 2000;
export const MAX_TEXTAREA_CONTENT_NORMAL_LENGTH = 1000;
export const MAX_TEXTAREA_CONTENT_MIDDLE_LENGTH = 500;
export const MAX_TEXTAREA_CONTENT_SHORT_LENGTH = 255;
export const MIN_INPUT_LENGTH = 3;
export const MAX_INPUT_LENGTH = 255;
export const MIN_NAME_LENGTH = 2;
export const MAX_NAME_LENGTH = 50;
export const MIN_NIKE_NAME_LENGTH = 3;
export const MAX_NIKE_NAME_LENGTH = 20;
export const MAX_EMAIL_LENGTH = 255;
export const MAX_PASSWORD_LENGTH = 72;
export const MIN_PASSWORD_LENGTH = 6;

export const REGEX_HAS_DIGITS = /\d/;
export const REGEX_HAS_LETTERS = /[A-Z]/;
export const REGEX_DIGITS = /^\d*$/;
export const REGEX_STRING = /^[a-zA-Zа-яА-ЯёЁіІїЇєЄґҐ\s]+$/i;
export const REGEX_MANE = /^[a-zA-Zа-яА-ЯёЁіІїЇєЄґҐ]+(?:['-][a-zA-Zа-яА-ЯёЁіІїЇєЄґҐ]+)*$/;
export const REGEX_USER_MANE = /^[a-zA-Z0-9_]+$/;
export const REGEX_PHONE_NUMBER = /^\+?[0-9\s\-()]{7,20}$/;
export const REGEX_TELEGRAM = /^@?[a-zA-Z0-9_]{5,32}$/;
export const REGEX_IMAGE_FORMATS = /\.(jpg|jpeg|png|webp|gif)$/i;
export const REGEX_IMAGE_BASE64_FORMATS = /^data:image\/(png|jpeg|jpg|webp);base64,/;

export const ALLOWED_LOCALES = ['en', 'ru', 'ua'];
